  ...
  return (
    <header>
      ...
      <nav>
        ...
        <div className="icone">
          <FontAwesomeIcon onClick={() => setCarrinho(!carrinho)} icon={faShoppingCart} size="lg"/>
        </div>
      </nav>
      {carrinho && 
      <div className="carrinho">
          ...
          {listaCarrinho.current.map(item => {
            return (
              <div className="card" key={item.productId}>
                <img src={item.image} alt="imagem" width="70px" height="70px"/>
                <div>
                  <p>{item.name}</p>
        ...
        <p className="valor-pedido">Total do pedido: <span>R$ 20.356,95</span></p>
